...
class Network_Monitor(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.monitor_thread = hub.spawn(self._monitor)###start the child processing        
        
	self.dbugfile = open("network_monitor_debugflie.txt",'w')
	
	print "Network_Monitor processing start"

###############################################################################################################################################	

    def _monitor(self):

	self.dbugfile.write("child processing start \n")
	self.dbugfile.flush()
	...
	isfirsttimecreate = True
        while True:
            ...
            for dp in self.datapaths.values():###reflash the portstat every SLEEP_PERIOD seconds
                self.port_link.setdefault(dp.id, {})
                self._request_stats(dp)

	    self.dbugfile.write("request port and flow stats over,now sleep 5s \n")
	    self.dbugfile.flush()

            hub.sleep(SLEEP_PERIOD)

	    self.dbugfile.write("hub sleep 5s end\n")
	    self.dbugfile.flush()

	    if self.stats['flow'] or self.stats['port']:
		self.dbugfile.write("self.stats['flow'] || self.stats['port'] is not null \n")
		self.dbugfile.flush()
		self.dbugfile.write("time"+str(time.time())+"\n")
		self.dbugfile.flush()			
	    else:
		self.dbugfile.write("self.stats['flow'] and self.stats['port'] is  null \n")
		self.dbugfile.flush()
	    	self.dbugfile.write("time"+str(time.time())+"\n")
		self.dbugfile.flush()

        self.dbugfile.write("child processing over \n")
	self.dbugfile.flush()  
 
###############################################################################################################################################

    def _request_stats(self, datapath):###request port and flow stats
    ...
    def show_stat(self, type, bodys):###do not use in this app
...
###############################################################################################################################################
    ...
    def  get_port_speed(self,switch_dpid,port_no):
	switch_dpid = int(switch_dpid)
	port_no = int(port_no)
	return abs(self.port_speed[(switch_dpid, port_no)][-1])  ###bytes/s
